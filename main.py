...
# General Functions
#-------------------------------------------------------------------------------
def BuildCutBox(staff_img_color,between_line,pos_Left,Width):
    CutImg = CutImage(staff_img_color,between_line,Width,pos_Left)
    return CutImg
...
def play_part(track, note, len_ ,note_bias=0,vel=1.0,delay=0,change=False,double=False):
    ...
    major_notes = [60,62,64,65,67,69,71,72]
    # C4 - 正中間的 DO
    vel = round(64*vel)
    t_start = round(delay*temple)
    t_end =  round(temple*len_)
    if not double:
        track.append(Message("note_on",  note=major_notes[note],velocity=vel,time=t_start))
        track.append(Message("note_off", note=major_notes[note],velocity=vel,time=t_end))
    if change:
    ...
    if double:
        ...
        track.append(Message("note_on",channel=1, note=major_notes[note],velocity=vel,time = t_start))
        track.append(Message("note_off",channel=1, note=major_notes[note],velocity=vel,time = t_end))
...
#使用霍夫变换
def getCorrect2(src,imgfile):
    
    #src = cv2.imread(imgfile,cv2.IMREAD_COLOR)
    # cv2.imshow("src",src)
    # cv2.waitKey(0)
    if not os.path.isdir(ROTATE_PATH):
        os.mkdir(ROTATE_PATH)    
    
        
    gray = cv2.imread(imgfile,cv2.IMREAD_GRAYSCALE)
    cv2.imwrite(os.path.join(ROTATE_PATH,"star_grey.png"),gray)
    ...
    cv2.imwrite(os.path.join(ROTATE_PATH,"star_eroDil.png"),eroDil)
    # cv2.imshow("eroDil",eroDil)
    # cv2.waitKey(0)

    #edge
    canny = cv2.Canny(eroDil,50,150)
    cv2.imwrite(os.path.join(ROTATE_PATH,"star_canny.png"),canny)
    ...
    # cv2.imwrite(os.path.join(ROTATE_PATH,"star_houghP.png"),drawing)
    ...
    cv2.imwrite(os.path.join(ROTATE_PATH,"star_fine.png"),rotateImg)
...
if __name__ == "__main__":
    ...
    img = getCorrect2(img,img_file)
    # cv2.imshow("window",img)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    # ============ Noise Removal ============
    # img = cv2.imread(os.path.join("Rotate","star_fine.png"),0)
    # img = sharpen(img)
    # cv2.imshow("window",img)
    # cv2.waitKey(0)
    ...
    # ============ Save every staff height ============
    #staff_y = [[0] * 2] * len(all_staffline_vertical_indices)
    between_line = all_staffline_vertical_indices[0][1][0]-all_staffline_vertical_indices[0][0][1]
    # staff_y = [[0 for i in range(2)] for j in range(len(all_staffline_vertical_indices))]
    # print(staff_y)


    # for i in range(len(all_staffline_vertical_indices)):
    #     # print(i)
    #     staff_y[i][0] = all_staffline_vertical_indices[i][0][0]
    #     # print(staff_y[i][0])
    #     lastLine = len(all_staffline_vertical_indices[i])-1
    #     staff_y[i][1] = all_staffline_vertical_indices[i][lastLine][1]
        
    #     #print(between_line)
    #     staff_y[i][0] = staff_y[i][0] - between_line*2
        
    #     staff_y[i][1] = staff_y[i][1] + between_line*2
        # print(staff_y[i][1])
    ...
    for i in range(len(staffs)):
        ...
        for box in flat_boxes:
            ...
            # pos_x,pos_y = box.getCorner()
            # wid = box.getWidth()
            # print("pos_x= ",str(pos_x))
            # print("wid= ",str(wid))
            # CutImg = BuildCutBox(Cut_img,between_line,pos_x,wid)
            # staff_primitives.append(quarter)
            # lsCutImage.append(CutImg)     
        ...
        lsCutImage.sort(key=lambda CutImg: CutImg.getCenter())
        for j in range(len(lsCutImage)):
            lsCutImage[j].saveImage(picIdx,"Cut")
            picIdx = picIdx + 1
        lsCutImage.clear()
...
pics = sorted(pics, key= lambda x: int(x.replace(".png", "")))
# print(pics)
...
BATCH_SIZE=32
dict_label = {}
model.load_state_dict(torch.load("Resnet18_5.pt", map_location=torch.device('cpu')))
valid_transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])
# resize成224的檔案路徑
TEST_DATA_PATH = 'Resize'
...
for i, pic in enumerate(pics):
  ...
  # vector = np.vectorize(np.int_)
    ...
    #print(f'{pic} = Predict_Label: {prediction}')
    # Picindex = Picindex +1
...
# 2.
with open('output/Train_Predict.csv', 'w', encoding = 'utf-8-sig') as csvfile:
  # 建立 CSV 檔寫入器
    writer = csv.writer(csvfile)
    writer.writerow(['pic', 'predict_label'])
    for key in dict_label:
    # 寫入一列資料
        lsMusicNote[iIndexNote]
        writer.writerow([key, dict_label[key]])
        iIndexNote = iIndexNote + 1
...
for Idx in lsMusicNote:
    track = play_part(track,Idx,1)

mid.tracks.append(track)
mid.save('output/train_song.mid')
